...
const projectSizes = ['Small', 'Medium', 'Large', 'Small Video', 'Medium Video', 'Large Video', 'Small Photo', 'Medium Photo', 'Large Photo', 'Small (fix) Web', 'Medium Web (landing page)', 'Web page feature changes', 'Web page feature change out of house PM', 'Website PM'];
const sizeCosts = [null, null, null, 500, 2000, 4000, 100, 200, 600, 150, 280, 1120, 5600, 14000];
...
function init() {
			...
			const request = https.request(options, (res) => {
				...
				res.on('end', () => {
					...
					if ( doc.documentElement.getElementsByTagName('wsse:BinarySecurityToken').item(0) ) { // eslint-disable-line
						...
						const text = document.createTextNode("Invalid Username/Password");
	...
	.then(() => {
		...
		let data = projectsBySize[currentYear].map((size, i) => {
			const label = sizeCosts[i] !== null ? `${projectSizes[i]} - $${sizeCosts[i]}` : projectSizes[i];
			return { label , value: size.length, color: color[i] };
		});
		data = data.filter(value => Object.keys(value).length !== 0);
		return data;
